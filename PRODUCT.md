...
**Roadmap (confirmed direction):** the all-Rust migration is complete — pi CLI RPC over stdio, shell/theme, session data, chat/markdown/tools, the workbench pages, and packaging all ship. From here the direction deepens the workbench rather than the single-session chat:

- **Workflow modes** — a session can start scoped as **Plan Mode**, **Build Mode**, or **Ask Mode** (plan, build, or answer), rather than one undifferentiated chat.
